...
func main() {
	...
	if err := installConfigMap("./externalyaml/configmap.yaml", mgr.GetClient()); err != nil {
		setupLog.Error(err, "unable to install zalando config-map")
		os.Exit(1)
	}
...
func installConfigMap(fileName string, client client.Client) error {
	bb, err := ioutil.ReadFile(fileName)
	if err != nil {
		return err
	}
	obj := &corev1.ConfigMap{}
	if _, _, err := serializer.NewCodecFactory(scheme).UniversalDeserializer().Decode(bb, nil, obj); err != nil {
		return err
	}
